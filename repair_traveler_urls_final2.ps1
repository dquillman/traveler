...
# stays_map_data (use a proper here-string with blank line start)
...
# tiny single-line stubs (avoid here-string pitfalls)
$stubCharts = 'def stays_charts(request):  return render(request, "stays/charts.html", {})'
$stubImport = 'def stays_import(request):  return render(request, "stays/import.html", {})'
$stubExport = 'def stays_export(request):  return render(request, "stays/export.html", {})'

if($views -notmatch 'def\s+stays_charts\s*\('){ $views += "`r`n`r`n$stubCharts`r`n" }
if($views -notmatch 'def\s+stays_import\s*\('){ $views += "`r`n$stubImport`r`n" }
if($views -notmatch 'def\s+stays_export\s*\('){ $views += "`r`n$stubExport`r`n" }
