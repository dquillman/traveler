...
def ensure_static_css():
    ...
    if not style_css.exists():
        style_css.write_text("/* placeholder so 404s stop; Tailwind CDN handles most UI */\n", encoding="utf-8")
...
def patch_settings():
    ...
    if "django.contrib.staticfiles" not in txt:
        # naive: insert into INSTALLED_APPS list
        txt = re.sub(
            r"(INSTALLED_APPS\s*=\s*\[)",
            r"\1\n    'django.contrib.staticfiles',",
            txt, count=1
        )
        print("• Added django.contrib.staticfiles to INSTALLED_APPS")
        changed = True

    # Ensure STATIC_URL
    if "STATIC_URL" not in txt:
        txt += "\n\nSTATIC_URL = '/static/'\n"
        print("• Added STATIC_URL = '/static/'")
        changed = True

    # Ensure BASE_DIR and STATICFILES_DIRS includes BASE_DIR / 'static'
    if "STATICFILES_DIRS" not in txt:
        if "BASE_DIR" not in txt:
            txt = "from pathlib import Path\nBASE_DIR = Path(__file__).resolve().parent.parent\n\n" + txt
            print("• Inserted BASE_DIR definition")
            changed = True
        txt += "\nSTATICFILES_DIRS = [BASE_DIR / 'static']\n"
        print("• Added STATICFILES_DIRS = [BASE_DIR / 'static']")
        changed = True
    else:
        # make sure 'static' is present
        if "BASE_DIR / 'static'" not in txt and 'BASE_DIR / "static"' not in txt:
            txt = re.sub(
                r"(STATICFILES_DIRS\s*=\s*\[)([^\]]*)\]",
                r"\1\2, BASE_DIR / 'static']",
                txt, count=1
            )
            print("• Appended BASE_DIR / 'static' to STATICFILES_DIRS")
            changed = True
