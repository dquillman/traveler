...
def appearance_view(request):
    """Render a simple Appearance settings page. Update the template as needed."""
    return render(request, "appearance.html")

# Additional views for import and export pages.
def import_view(request):
    """Render the import page for stays. Expects a template at templates/stays/import.html."""
    return render(request, "stays/import.html")

def export_view(request):
    """Render the export page for stays. Expects a template at templates/stays/export.html."""
    return render(request, "stays/export.html")

# API endpoint for map data. Returns JSON for stays with lat/lng.
from django.http import JsonResponse

def stays_map_data(request):
    """Return JSON data for all stays with latitude and longitude.

    This allows the frontend to fetch map points dynamically.
    """
    qs = _apply_stay_filters(Stay.objects.all(), request)
    qs = qs.exclude(latitude__isnull=True).exclude(longitude__isnull=True)
    points = []
