# fix_traveler_all.ps1
# One-pass repair for: Export, Charts, Map, Import (adds file chooser)
# Creates backups of each edited file with .bak.TIMESTAMP

$ErrorActionPreference = "Stop"

function Backup($path) {
  if (Test-Path $path) { Copy-Item $path "$path.bak.$(Get-Date -Format 'yyyyMMdd_HHmmss')" -Force }
}
...
# =========================
# FORMS: ensure StayImportForm (file input)
# =========================
...
# =========================
# VIEWS: add/ensure all endpoints
# =========================
...
if ($need.Count -gt 0) { $views = ($need -join "`r`n") + "`r`n" + $views }
...
# Map views (Leaflet: map page + data endpoint)
if ($views -notmatch "def\s+map_page\(") {
  $views += @"
...
def stays_map_data(request):
    # Returns simple GeoJSON FeatureCollection from Stay latitude/longitude
    features = []
...
# Import view (robust encoding + file upload)
...
def import_stays(request):
    if request.method == 'POST':
        ...
        if form.is_valid() and 'file' in request.FILES:
            ...
            return redirect('stays:stays_import')  # safe even if stay_list missing
    else:
        form = StayImportForm()
...
$urls = [regex]::Replace($urls, "(?s)(urlpatterns\s*=\s*\[)(.*?)(\])", {
  ...
  foreach ($n in @('stays_export','stays_export_csv','stays_charts','stays_chart_data','stays_map','stays_map_data','stays_import')) {
    if ($mid -notmatch [regex]::Escape($n)) { $mid = $mid.TrimEnd() + "`r`n" + $routes; break }
  }
  return $pre + $mid + $post
})
...
# =========================
# TEMPLATES (standalone pages that work even without base.html)
# =========================
...
# MAP (Leaflet + working data endpoint)
Backup $mapHtml
@'
...
# IMPORT (explicit file chooser with enctype)
Backup $importHtml
@'
...
Write-Host "`n✅ All endpoints & pages installed."
Write-Host "   • Export: /stays/export/ (uses namespaced URL stays:stays_export_csv)"
Write-Host "   • Charts: /stays/charts/ (Chart.js CDN)"
Write-Host "   • Map:    /stays/map/ (Leaflet + /stays/map/data/)"
Write-Host "   • Import: /stays/import/ (file chooser visible)"
