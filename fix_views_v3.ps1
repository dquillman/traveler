<# 
fix_views_v3.ps1
- Backs up stays\views.py
- Replaces 3 functions with clean versions: _apply_stay_filters, stay_list, stay_add, stay_edit
- If a function isn't found, it inserts it right before stay_list
#>
...
function Replace-PyDef {
    ...
    # Match from "def FuncName(...):" through the block up to next top-level "def" or EOF
    $pattern = "(?ms)^[ \t]*def[ \t]+$([regex]::Escape($FuncName))[ \t]*\([^\)]*\)[ \t]*:[\s\S]*?(?=^[ \t]*def[ \t]+\w+[ \t]*\(|\Z)"
    if ([regex]::IsMatch($Source, $pattern)) {
        return [regex]::Replace($Source, $pattern, [System.Converter]::ToString($Replacement))
    } else {
        return $null
    }
}
...
if ($null -ne $updated) {
    ...
    # Insert before stay_list if absent
    $insertBefore = "(?m)^[ \t]*def[ \t]+stay_list[ \t]*\("
    if ([regex]::IsMatch($text, $insertBefore)) {
        $text = [regex]::Replace($text, $insertBefore, [System.Converter]::ToString($replacement_filters) + "`r`n`r`n" + "def stay_list(", 1)
        Write-Host "Inserted: _apply_stay_filters before stay_list"
    } else {
        # If stay_list also missing, append at end
        $text = $text.TrimEnd() + "`r`n`r`n" + $replacement_filters + "`r`n"
        Write-Host "Appended: _apply_stay_filters at end of file"
    }
}
...
if ($null -ne $updated) {
    ...
    Write-Host "Appended: stay_add (was missing)"
}
...
if ($null -ne $updated) {
    ...
    $text = $text.TrimEnd() + "`r`n`r`n" + $replacement_edit + "`r`n"
    Write-Host "Appended: stay_edit (was missing)"
}
...
if ($py) {
    Write-Host "Running python -m py_compile stays\views.py"
    ...
    if ($proc.ExitCode -eq 0) {
        ...
        Write-Warning "python -m py_compile repor_
