...
# 2) Un-glue any ')def ...' stuck together
$t = $t -replace "\)\s*def\s+", ")`r`n`r`ndef "

# 3) Replace/ensure robust stays_map_data()
$staysMapData = @'
...
if ($t -match "def\s+stays_map_data\s*\(") {
  $t = [regex]::Replace($t, "def\s+stays_map_data\s*\([^\0]*?(?=^def\s|\Z)", $staysMapData, "Singleline, Multiline")
} else {
  $t = $t.TrimEnd() + "`r`n`r`n" + $staysMapData + "`r`n"
}

# 4) Ensure map_page() exists (clean here-string terminator)
if ($t -notmatch "def\s+map_page\s*\(") {
$mapPage = @'
def map_page(request):
    return render(request, "stays/map.html")
'@
  $t = $t.TrimEnd() + "`r`n`r`n" + $mapPage + "`r`n"
}

Set-Content -Path $views -Value $t -Encoding UTF8
Write-Host "✅ stays/views.py fixed (syntax + robust stays_map_data + map_page)."
