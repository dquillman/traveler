# doctor_fix.ps1
# Replaces stays_doctor.ps1 with a non-interactive, clean JSON version.

$ErrorActionPreference = "Stop"

# 1) Detect DJANGO_SETTINGS_MODULE from manage.py
...
# 2) Write the new stays_doctor.ps1
@"
param(
  [switch]`$Fix
)
...
# ---- Write a standalone Python probe (no manage.py shell) ----
...
# ---- Run the probe cleanly ----
`$out = & python `"$probePath`" 2>&1
if (`$LASTEXITCODE -ne 0 -or -not `$out) {
  Write-Host "Probe failed. Output:"; Write-Host `$out; throw "Probe failed"
}

try {
  `$info = `$out | ConvertFrom-Json
except {
  Write-Host "JSON parse failed. Raw output:"; Write-Host `$out; throw
}
...
Write-Host ""
Write-Host "Stay fields:"
`$stayFields | ForEach-Object { Write-Host " - $_" }
Write-Host ""
Write-Host "URL names:"
`$urlNames | ForEach-Object { Write-Host " - $_" }
Write-Host ""
Write-Host "stays.views attributes:"
`$viewsAttrs | ForEach-Object { Write-Host " - $_" }
...
if (Test-Path `$templatePath) {
  ...
  `$referenced = @()
  foreach (`$m in `$fieldMatches) { `$referenced += `$m.Groups[1].Value }
  ...
  `$urlRefs = @()
  foreach (`$m in `$urlMatches) { `$urlRefs += `$m.Groups[1].Value }
  ...
  if (`$referenced.Count -gt 0) { Write-Host ("Referenced stay.<field>: " + (`$referenced -join ", ")) }
  if (`$unknownStayFields.Count -gt 0) { Write-Host ("Unknown fields in template: " + (`$unknownStayFields -join ", ")) } else { Write-Host "No unknown stay fields in template." }
  if (`$unknownUrlNames.Count -gt 0) { Write-Host ("Missing URL names in template: " + (`$unknownUrlNames -join ", ")) } else { Write-Host "All template URL tags resolve to known names." }
} else {
  Write-Host ""
...
# Ensure alias 'list' exists (reverse('list'))
...
# Add stubs for legacy views if missing
...
"@ | Set-Content "stays_doctor.ps1" -Encoding UTF8

Write-Host "Wrote stays_doctor.ps1 (non-interactive probe; clean JSON)."
Write-Host "Run: .\stays_doctor.ps1   or   .\stays_doctor.ps1 -Fix"
